...
type PositionGroupsDestroyParams struct {
	DestroyMembers bool `json:"destroy_members"`
}
...
func (c PositionGroupsController) Destroy(resp http.ResponseWriter, req *http.Request, params map[string]string) error {
	...
	var destroyParams PositionGroupsDestroyParams
	err = json.NewDecoder(req.Body).Decode(&destroyParams)
	if err != nil {
		utils.Err(ctx, resp, http.StatusBadRequest, err.Error())
		return nil
	}
	...
	for _, position := range positions {
		if destroyParams.DestroyMembers {
			err := document.Destroy(ctx, models.PtzPositionCollection, &position)
			if err != nil {
				return errors.Wrap(err, "fail to destroy member")
			}
		} else {
			...
			if err != nil {
				return errors.Wrap(err, "fail to update member")
			}
		}
	}
